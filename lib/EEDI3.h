...
using aligned_float =
    std::unique_ptr<float[], decltype(&vsh::vsh_aligned_free)>;
...
struct EEDI3Data final {
    ...
    void (*filter)(const VSFrame* src, const VSFrame* scp, const VSFrame* mclip,
                   VSFrame* mcp, VSFrame** pad, VSFrame* dst, void* srcVector,
                   uint8_t* mskVector, bool* VS_RESTRICT bmask, int* pbackt,
                   int* VS_RESTRICT fpath, int* dmap, const int field_n,
                   const EEDI3Data* VS_RESTRICT d, const VSAPI* vsapi) noexcept;
};

static void copyMask(const VSFrame* src, VSFrame* dst, const int plane,
                     const bool dh, const int field_n,
    ...
    vsh::bitblt(
        ...
        vsapi->getStride(src, plane) * mul, vsapi->getFrameWidth(src, plane),
        vsapi->getFrameHeight(dst, plane));
}

template <typename pixel_t>
static void copyPad(const VSFrame* src, VSFrame* dst, const int plane,
    ...
    const pixel_t* srcp =
        reinterpret_cast<const pixel_t*>(vsapi->getReadPtr(src, plane));
    pixel_t* VS_RESTRICT dstp =
        reinterpret_cast<pixel_t*>(vsapi->getWritePtr(dst, 0));

    if (!dh)
        vsh::bitblt(dstp + dstStride * (MARGIN_V + off) + MARGIN_H,
                    vsapi->getStride(dst, 0) * 2, srcp + srcStride * off,
                    vsapi->getStride(src, plane) * 2,
                    srcWidth * sizeof(pixel_t), srcHeight / 2);
    else
        vsh::bitblt(dstp + dstStride * (MARGIN_V + off) + MARGIN_H,
                    vsapi->getStride(dst, 0) * 2, srcp,
                    vsapi->getStride(src, plane), srcWidth * sizeof(pixel_t),
                    srcHeight);
...
template <typename pixel_t>
static inline void
interpolate(const pixel_t* src3p, const pixel_t* src1p, const pixel_t* src1n,
            const pixel_t* src3n, pixel_t* VS_RESTRICT dstp, const bool* bmask,
            const int* fpath, int* VS_RESTRICT dmap, const int width,
            const bool ucubic, const int peak) noexcept {
    for (int x = 0; x < width; x++) {
        if (bmask && !bmask[x]) {
            ...
            if (ucubic)
                dstp[x] = std::clamp(
                    (9 * (src1p[x] + src1n[x]) - (src3p[x] + src3n[x]) + 8) /
                        16,
                    0, peak);
            else
            ...
            if (ucubic && x >= absDir3 && x <= width - 1 - absDir3)
                dstp[x] = std::clamp((9 * (src1p[x + dir] + src1n[x - dir]) -
                                      (src3p[x + dir3] + src3n[x - dir3]) + 8) /
                                         16,
                                     0, peak);
            else
...
template <>
inline void
interpolate(const float* src3p, const float* src1p, const float* src1n,
            const float* src3n, float* VS_RESTRICT dstp, const bool* bmask,
            const int* fpath, int* VS_RESTRICT dmap, const int width,
            const bool ucubic, [[maybe_unused]] const int peak) noexcept {
    for (int x = 0; x < width; x++) {
        if (bmask && !bmask[x]) {
            dmap[x] = 0;

            if (ucubic)
                dstp[x] = 0.5625f * (src1p[x] + src1n[x]) -
                          0.0625f * (src3p[x] + src3n[x]);
            else
            ...
            if (ucubic && x >= absDir3 && x <= width - 1 - absDir3)
                dstp[x] = 0.5625f * (src1p[x + dir] + src1n[x - dir]) -
                          0.0625f * (src3p[x + dir3] + src3n[x - dir3]);
            else
...
static inline void prepareLines(const pixel_t* srcp, vector_t* _dstp,
                                const int width, const int height,
                                const ptrdiff_t srcStride,
                                const ptrdiff_t dstStride, const int srcY,
...
static inline void prepareMask(const uint8_t* srcp, uint8_t* VS_RESTRICT dstp,
                               const int width, const int height,
                               const ptrdiff_t stride, const int srcY,
                               const int vectorSize) noexcept {
...
template <typename pixel_t>
static void vCheck(const pixel_t* srcp, const pixel_t* scpp,
                   pixel_t* VS_RESTRICT dstp, const int* dmap, void* _tline,
                   const int field_n, const int width, const int height,
                   const ptrdiff_t srcStride, const ptrdiff_t dstStride,
    ...
    for (int y = MARGIN_V + field_n; y < height - MARGIN_V; y += 2) {
        if (y >= 6 && y < height - 6) {
            ...
            for (int x = 0; x < width; x++) {
                ...
                const int vt = std::abs(dst2p[x + dirc] - dst1p[x + dirc]) +
                               std::abs(dstp[x + dirc] - dst1p[x + dirc]);
                const int vb = std::abs(dst2n[x - dirc] - dst1n[x - dirc]) +
                ...
                const int mdiff0 = (d->vcheck == 1)
                                       ? std::min(d0, d1)
                                       : (d->vcheck == 2 ? (d0 + d1 + 1) / 2
                                                         : std::max(d0, d1));
                const int mdiff1 = (d->vcheck == 1)
                                       ? std::min(d2, d3)
                                       : (d->vcheck == 2 ? (d2 + d3 + 1) / 2
                                                         : std::max(d2, d3));

                const float a0 = mdiff0 * d->rcpVthresh0;
                const float a1 = mdiff1 * d->rcpVthresh1;
                const float a2 = std::max(
                    (d->vthresh2 - std::abs(dirc)) * d->rcpVthresh2, 0.0f);
                const float a = std::min(std::max({a0, a1, a2}), 1.0f);

                tline[x] =
                    static_cast<pixel_t>((1.0f - a) * dstp[x] + a * cint);
            }
...
template <>
void vCheck(const float* srcp, const float* scpp, float* VS_RESTRICT dstp,
            const int* dmap, void* _tline, const int field_n, const int width,
    ...
    for (int y = MARGIN_V + field_n; y < height - MARGIN_V; y += 2) {
        if (y >= 6 && y < height - 6) {
            ...
            for (int x = 0; x < width; x++) {
                const int dirc = dmap[x];
                const float cint = scpp ? scpp[x]
                                        : 0.5625f * (dst1p[x] + dst1n[x]) -
                ...
                const float mdiff0 = (d->vcheck == 1)
                                         ? std::min(d0, d1)
                                         : (d->vcheck == 2 ? (d0 + d1) / 2.0f
                                                           : std::max(d0, d1));
                const float mdiff1 = (d->vcheck == 1)
                                         ? std::min(d2, d3)
                                         : (d->vcheck == 2 ? (d2 + d3) / 2.0f
                                                           : std::max(d2, d3));
